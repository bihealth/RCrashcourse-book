...
`readxl`, is used for Excel files and is a separate package that you need
to install and load separately.
...
[tibbles]{.aside}
The various tidyverse functions return *tibbles*, which, as you remember from
...
As you can see, the functions I recommend to use can be used by loading
...
Getting "up one level" in a path is done by using `..` (two dots). So the relative
 ...
 * Save the files in the `Datasets/` directory in your working directory – or
...
this is not the way to go – what if you have not 150 values, but 150,000?
[`is.na()`]{.aside}
...
all other characters are also allowed (so a column name can be `豚肉 😀 "temp."? (°C)`), it
...
The last column in the table above is our target – this is how we would
...
What is the problem
on line 70? A comma instead of a decimal dot – maybe someone who typed it
...
Python. As you may guess, they have a different syntax then R – they are
...
[regexp rules]{.aside}
A regular expression is a sequence of characters – a string. However, some
...
what you call in German *gewöhnungsbedürftig* – they require some getting
...
special characters, we must *escape* them – which in R means putting two
 ...
 * When using Excel to enter data, [read this first](https://cghlewis.com/blog/excel_entry/).

This latter point warrants an explanation. There are two main problems with
that format. Firstly, it is less portable then XLSX (the new one) – so many
...
everyday work with data – not only in R, but in general.
...
somewhere. However, chances are that you were wrong – maybe that *was* a
