...
are also two other pre-defined values, `T` and `F`, which are equivalent to `TRUE`
and `FALSE`, respectively. **Do not use them**. Unlike `TRUE` and `FALSE`,
the values `T` and `F` can be overwritten, which will inevitably lead to chaos and mayhem.
:::
...
They are vectorized like any other arithmetic operators. Their result
...
containing as many elements as there were in the vector `numbers`:
...
Note that `0 > 0` is `FALSE`, not `TRUE`.

Now we can use the resulting
logical vector to select elements from the original vector:
...
Note: in the code above, we have sequentially filtered for numbers greater
than 0.5, and then numbers smaller than 1.0. However, typically you would
combine these two conditions into one using the `&` operator, which I
will introduce on Day 4.
...
`paste0()`. I am introducing them here not because they are so frequently
used for creating column and row names, but because they are very
frequently used, full stop.
[`paste0()`,`rep()`]{.aside}
...
```{r}
# first, create vectors as before
n <- length(LETTERS)
abc3 <- rep(LETTERS, n)
a3b3c3 <- rep(LETTERS, each = n)

dublets <- paste0(a3b3c3, abc3)

# now, reapeat each dublet n times
# those are the first two letters of our code
dublets <- rep(dublets, each = n)

# for the third letter, repeat each letter n^2 times
# and paste it to the dublets
col_names <- paste0(dublets, rep(LETTERS, n^2))
...
Note: *actually*, thanks to R recycling, you do not need to repeat the
    `dublets` vector `n^2` times. R will repeat it for you. However, I
    advice you not to rely on R recycling – at least not without double
    checking.
...
However, if you ask what the names of the elements of the list are, you
will get an answer:
...
And, of course, you can access the elements of the list in the usual way:
...
Lists are one of the most frequently used data structures in R. You will
see them a lot in everydays work with R.
...
You create data frames with the `data.frame()` function.
