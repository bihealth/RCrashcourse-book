...
console, it *does not look like one*. This is because R has a special
...
::: {.callout-caution icon=false appearance="simple"}
::: {#exr-paste-rep}
Use `install.packages` to install the package `colorDF` from CRAN. Then,
load the package using `library(colorDF)`. What does that package do? How
can you check that? (Hint: use `??colorDF`).
:::
:::
