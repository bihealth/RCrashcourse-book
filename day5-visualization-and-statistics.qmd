...
R markdown is a remarkably simple to use and yet powerfull tool. The basic idea is as
...
to make a single system that would be good for all of them. But the main
...
is chosen with a single command. There is no need to separately change
colors for the legend and the plot. The `theme_minimal()` function changes a
number of parameters such as the font face used or the background color.
...
The data set `Datasets/expression_data_vaccination_example.xlsx` contains
over 8 thousand genes for over 120 samples. Luckily for us, the data is
pretty much cleaned up and ready for analysis.
...
meta <- read_excel("Datasets/expression_data_vaccination_example.xlsx",
                   sheet="targets")
dim(meta)
trns <- read_excel("Datasets/expression_data_vaccination_example.xlsx",
                   sheet="expression") |>
        as.matrix()
dim(trns)
all(meta$ID == colnames(trns))
```

The `meta` data frame contains the sample information, and the `trns` data
holds the actual data. We have already converted the expression data to a
matrix. Before we run the PCA, however, we need to *transpose* it, that is,
flip the dimensions. PCA wants the variables in columns, and samples in
rows; however, the `trns` has the variables (genes) in rows and samples in
columns. Note that we have checked that the column names of the `trns` data
are identical to the `ID` column of the `meta` data frame.
...
`cbind()` to combine the `meta` data frame with the first few columns of
the `pca$x` matrix. We will use the `ggplot2` package to plot the data.
[`cbind()`]{.aside}

```{r}
library(ggplot2)
pca_df <- cbind(meta, pca$x[,1:10]) |>
  filter(Timepoint == "D1")
ggplot(pca_df, aes(x=PC5, y=PC6, color=b.a.t)) +
  geom_point()
```
...
Here is a ggplot2 code that produces a bar chart.
...
**Paired t-test.** The paired t-test is a special case of the t-test where
...
use a regular t-test if the data is paired**, because one of the most
fundamental and important assumptions is not met. Also, in many cases,
using a paired t-test will give you more statistical power.

Consider this example. We first start with randomized vector `x`, then
build vector `y` by adding a fixed effect and an error term:

[`boxplot()`]{.aside}

```{r}
x <- rnorm(15, mean=1, sd=1)
y <- x + 0.5 + rnorm(15, mean=0, sd=.5)
boxplot(x, y)
...
a p-value of 
`r format.pval(t.test(x, y, paired=TRUE)$p.value, digits=2)`, whereas with
regular t-test we would have gotten a p-value of
`r format.pval(t.test(x, y)$p.value, digits=2)`.

Incidentally, we produced a boxplot. We will talk about boxplots in a
moment.
 ...
 * [StackOverflow](https://stackoverflow.com) – this is a general Q & A
   forum for software developers. Most of the questions you have someone
   has previously asked here.
 * Online R books
